...
togglbutton.render(
  '[data-item-detail-root] [data-item-actions-root]:not(.toggl)',
  { observe: true },
  (elem) => {
    const description = () => elem.dataset.itemContent || ''
    const project = () => elem.dataset.itemProjectName || ''
...
togglbutton.render(
  ...
  (elem) => {
    ...
    if (document.querySelector('.project_view header h1')) {
      project = document
        .querySelector('.project_view header h1')
        .textContent.trim()
    } else if (document.getElementById(`item_${projectId}`)) {
      // (legacy?) project ID element
      const projectContent = document
        .getElementById(`item_${projectId}`)
