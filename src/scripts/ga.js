...
var GA = {
  ...
  reportSettings: function(event, service) {
    this.report(
      'start-automatically',
      'settings/start-automatically-' + Db.get('startAutomatically')
    );
    this.report(
      'stop-automatically',
      'settings/stop-automatically-' + Db.get('stopAutomatically')
    );
    this.report(
      'right-click-button',
      'settings/show-right-click-button-' + Db.get('showRightClickButton')
    );
    this.report('popup', 'settings/popup-' + Db.get('showPostPopup'));
    this.report('reminder', 'settings/reminder-' + Db.get('nannyCheckEnabled'));
    this.report(
      'reminder-minutes',
      'settings/reminder-minutes-' + Db.get('nannyInterval')
    );
    this.report(
      'idle',
      'settings/idle-detection-' + Db.get('idleDetectionEnabled')
    );

    this.report(
      'pomodoro',
      'settings/pomodoro-' + Db.get('pomodoroModeEnabled')
    );
    if (Db.get('pomodoroModeEnabled')) {
      this.report(
        'pomodoro-sound',
        'settings/pomodoro-sound-' + Db.get('pomodoroSoundEnabled')
      );
      if (Db.get('pomodoroSoundEnabled')) {
        this.report(
          'pomodoro-volume',
          'settings/pomodoro-volume-' + Db.get('pomodoroSoundVolume')
        );
      }
      this.report(
        'pomodoro-stop',
      ...
      this.report(
        'pomodoro-interval',
        'settings/pomodoro-interval-' + Db.get('pomodoroInterval')
      );
    }

    this.report(
      'stop-at-day-end',
      'settings/stop-at-day-end' + Db.get('stopAtDayEnd')
    );
    if (Db.get('stopAtDayEnd')) {
      this.report(
        'stop-at-day-end-time',
        'settings/stop-at-day-end-time' + Db.get('dayEndTime')
      );
    }
    this.report(
      'remember-project-per',
      'settings/remember-project-per-' + Db.get('rememberProjectPer')
    );
