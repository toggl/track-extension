...
togglbutton.render(
  ...
  function (elem) {
    ...
    if (issueWrapper) {
      ...
      container = elem.querySelector('#ak-project-view-navigation')
...
// Jira sidebar/compact layout (board panel view)
togglbutton.render(
  '[data-testid="issue.views.issue-details.issue-layout.compact-layout"]:not(.toggl)',
  { observe: true },
  function (elem) {
    const issueWrapper = elem.querySelector(
      '[data-testid="issue.views.issue-base.foundation.breadcrumbs.breadcrumb-current-issue-container"]',
    ...
    if (!issueWrapper) return

    const container = issueWrapper.parentElement

    if (container.querySelector('.toggl-button')) {
      if (process.env.DEBUG) {
        console.info('🚫 "Jira compact layout sidebar" quit rendering early')
      }
      return
    }

    if (process.env.DEBUG) {
      console.info('🏃 "Jira compact layout sidebar" rendering')
    }

    try {
      const link = togglbutton.createTimerLink({
        className: 'jira2018',
        description: getDescription(issueWrapper),
        projectName: getProject,
        container:
...
function getProject() {
  ...
  projectElement =
    $('[data-testid="rapidboard-breadcrumbs"] a[href*="browse"] span') ||
    $('[data-test-id="rapidboard-breadcrumbs"] a[href*="browse"] span')
  ...
  if (projectElement) {
    const projectWrapper = projectElement.parentElement.querySelector(
      'a[href*="browse"] span + span',
    )
